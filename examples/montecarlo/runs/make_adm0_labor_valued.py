"""ADM0 valued labor from the adm1 reduced frame.

The adm1 pipeline runs keep their reduced frame (one row per target,
window, and sample member). The valued effect is extensive, so summing
the frame by ISO before pooling gives ADM0 exactly: impact regions do
not cross national borders and both sides use the same weight file.
Statistics cannot be summed, which is why this works from the reduced
frame and not from the statistics file.

Run after the adm1 pipeline run for the same rcp:

    python make_adm0_labor_valued.py --rcp rcp85
"""
...
BASE = Path("/project/cil/gcp/impact_map/labor")
...
def main() -> int:
    ...
    reduced_path = BASE / "adm1" / f"labor_valued_{args.rcp}.reduced.parquet"
    ...
    (out / f"{stem}.manifest.json").write_text(json.dumps(
        {"derived_from": str(reduced_path),
         ...
         "units": "2005 PPP USD, fulladapt minus histclim",
         "quantiles": QUANTILES,
